...
var Log = function() {
    console.log(chalk.grey('GsClient') + ' ' + Array.prototype.join.call(arguments, '    '));
}

function GsClient(clientSocket) {
    var _this = this;
    ...
    this.socket.on('end', function(err) {
        ...
        Log('Connection Closed', _this.socket.remoteAddress);
    });
    ...
    this.writeError = function(code, message) {
        ...
        this.write(('\\error\\\\err\\' + code+ '\\fatal\\\\errmsg\\' + messagef + '\\id\1\\final\\"'))
    }
